...
import { samplePath } from "../v3/pathSample.js";
import { complexity } from "./export.js";
import type { GeometricPrimitive, PatternPrimitive, ScenePrimitive, VectorScene } from "./types.js";
...
export interface SemanticGate {
  pass: boolean;
  perPart: { id: string; paths: number; precision: number; recall: number; iou: number }[];
...
/** 선만 남긴 SVG — fill 로 그려지는 표현(outline·면)도 잉크이므로 그대로 둔다 */
function lineOnly(scene: VectorScene, svg: string): string {
  // FACE_FILL 만 지운다. 그것은 면이고, 나머지(outline·패턴·프리미티브·stroke)는 전부 선이다.
  return svg.replace(new RegExp('<path[^>]*data-cls="FACE_FILL"[^>]*/>', "g"), "");
}
...
export async function runQa4(
  ...
  const vec = await svgInkMask(lineOnly(scene, svgs.fidelity), W, H);
  ...
  const fidelityGate: FidelityGate = {
    pass: lineFid.f.f2 >= th.f2 && detail.recall >= th.detail && Math.abs(lineFid.inkRatio - 1) <= 0.25,
  ...
  let contourLen = 0, shortPaths = 0, pathCount = 0;
  const minLen = Math.min(W, H) * 0.01;
  for (const p of scene.primitives) {
    const d = (p as { d?: string }).d;
    if (!d) continue;
    pathCount++;
    const pts = samplePath(d, 4, 200);
    let L = 0;
    for (let i = 1; i < pts.length; i++) L += Math.hypot(pts[i].x - pts[i - 1].x, pts[i].y - pts[i - 1].y);
    contourLen += L;
    if (L < minLen) shortPaths++;
  }
  ...
  const shortRatio = pathCount ? shortPaths / pathCount : 0;
  if (shortRatio > 0.35) eNotes.push(`짧은 패스 ${(shortRatio * 100).toFixed(0)}% — 파편화 의심`);
  if (!prims.length) eNotes.push("기하 프리미티브로 승격된 성분 없음 — 잔차 임계 확인");

  const editabilityGate: EditabilityGate = {
    pass: anchorDensity <= th.anchorDensity && shortRatio <= 0.35,
    paths: cf.paths, anchors: cf.anchors, uses: cf.uses, kb: cf.kb,
  ...
  for (const part of scene.parts) {
    ...
    if (!mine.length) {
      if (sharedWith.has(part.id) || (part.occludedBy?.length ?? 0) >= 2) sharedOnlyParts.push(part.id);
      else emptyVisibleParts.push(part.id);
      perPart.push({ id: part.id, paths: 0, precision: -1, recall: -1, iou: -1 });
      continue;
    }
    const mask = maskById.get(part.id);
    if (!mask) { perPart.push({ id: part.id, paths: mine.length, precision: -1, recall: -1, iou: -1 }); continue; }
    const layer = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${W} ${H}" width="${W}" height="${H}">` +
      mine.map((p) => {
        const d = (p as { d?: string }).d;
        if (!d) return "";
        return p.cls === "STRUCTURAL_STROKE" || p.cls === "DASH_OR_STITCH" || p.cls === "GEOMETRIC_PRIMITIVE"
          ? `<path d="${d}" fill="none" stroke="#000" stroke-width="${(p as { width?: number }).width ?? 2}"/>`
          : `<path d="${d}" fill="#000"/>`;
      }).join("") + "</svg>";
    const one = await svgInkMask(layer, W, H, 200);
  ...
  const meanPrecision = measured.length ? measured.reduce((s, p) => s + p.precision, 0) / measured.length : 1;
  ...
  if (ctx.aspectRatio > 1.02) sNotes.push(`사진↔도면 종횡비 불일치 ${((ctx.aspectRatio - 1) * 100).toFixed(1)}% — 배분 정확도에만 영향`);
  ...
  const semanticGate: SemanticGate = {
    pass: meanPrecision >= th.precision && emptyVisibleParts.length === 0,
    perPart, meanPrecision: +meanPrecision.toFixed(4), misassigned,
    emptyVisibleParts, sharedOnlyParts,
